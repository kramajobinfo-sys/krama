function App() {
  ...
  React.useEffect(() => {
    ...
    if (deepPage && PAGE_LINKS.indexOf(deepPage) !== -1) { setPage(deepPage); }
    ...
    else if (_mInfo) { setPage(_mInfo[1]); }
    ...
    Promise.allSettled([
      ...
      (_mJobSlug && !deepJobId)
        ? api.fetchJobDetail(_mJobSlug[1])
            .then((j) => { if (j && j.id) openJob(api.normaliseJob(j)); else setPage("jobs"); })
            .catch(() => setPage("jobs"))
        : Promise.resolve(),
  ...
  React.useEffect(() => {
    ...
    else if (page === "privacy" || page === "terms") desired = "/" + page;
  ...
  React.useEffect(() => {
    const onPop = () => {
      ...
      } else if (mInfo) { setJob(null); setPage(mInfo[1]); }
