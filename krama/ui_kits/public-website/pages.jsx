  ...
  function PricingSection({ onNav }) {
    ...
    return (
      <div style={{ display: "flex", flexDirection: "column", gap: 20 }}>
        ...
        <div className="krm-info-grid krm-pricing-grid" style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(210px, 1fr))", gap: 16, alignItems: "start" }}>
          {plans.map((p, i) => {
  ...
  function InfoPage({ slug, onNav }) {
    ...
    return (
      <div style={{ background: "var(--surface-page)", minHeight: "70vh" }}>
        ...
        {slug === "employers"
          ? (() => { const h = loadBanner("employersHero", { heading: c.title, sub: c.lead, image: "../../assets/banners/bg-employersHero.svg", fit: "cover", imgOverlay: 45 }); const hasImg = !!h.image; const showTxt = !h.hideText;
            const txt = (
          ...
          : (() => { const h = loadBanner(slug + "Hero", { heading: c.title, sub: c.lead, image: "", fit: "cover", imgOverlay: 45 }); const hasImg = !!h.image; const showTxt = !h.hideText;
            const txt = (
              <div className={h.hideOnMobile ? "krm-hero-hide-mobile" : undefined} style={{ maxWidth: 1200, margin: "0 auto", width: "100%", padding: hasImg ? "0 32px" : 0 }}>
                <div style={{ fontSize: "var(--text-xs)", fontWeight: 800, letterSpacing: "0.1em", textTransform: "uppercase", color: "var(--teal-200)" }}>{TR(c.eyebrow)}</div>
