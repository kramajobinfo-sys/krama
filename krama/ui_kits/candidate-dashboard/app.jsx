...
(function init() {
  ...
  const { Button, Badge, Avatar, Card, StatCard, Tabs, ProgressTracker, JobCard, EmptyState, Input, Textarea, Select: DSSelect, Tag, Switch, Modal } = NS;
      ...
      <div className="krm-page-pad" style={{ padding: 28 }}>
        <div style={{ maxWidth: 740 }}>
          <div style={{ marginBottom: 22, display: "flex", alignItems: "flex-start", justifyContent: "space-between", gap: 12 }}>
            ...
            {isPremium && <span style={{ flexShrink: 0, display: "inline-flex", alignItems: "center", gap: 5, background: "linear-gradient(180deg,#F7CE63,#D99A1F)", color: "#4a3300", fontWeight: 700, fontSize: "var(--text-xs)", padding: "5px 11px", borderRadius: 999 }}>{I("crown", 13)} {T("Premium")}</span>}
          </div>

          {viewers.length === 0 && locked === 0 ? (
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
              ...
              {locked > 0 && (
                <div style={{ position: "relative", marginTop: 2 }}>
                  {/* Blurred, non-identifying placeholder rows behind the upgrade CTA */}
                  <div style={{ display: "flex", flexDirection: "column", gap: 10, filter: "blur(3px)", opacity: 0.55, pointerEvents: "none", userSelect: "none" }} aria-hidden="true">
                    {Array.from({ length: Math.min(locked, 3) }).map(function(_, i) {
                      return (
                        <Card key={i} style={{ display: "flex", alignItems: "center", gap: 16 }}>
                          <div style={{ width: 48, height: 48, borderRadius: 10, background: "var(--surface-sunken, #e9ecef)", flexShrink: 0 }} />
                          <div style={{ flex: 1 }}>
                            <div style={{ height: 12, width: "42%", background: "var(--surface-sunken, #e9ecef)", borderRadius: 6 }} />
                            <div style={{ height: 10, width: "60%", background: "var(--surface-sunken, #eef1f3)", borderRadius: 6, marginTop: 8 }} />
                          </div>
                        </Card>
                      );
                    })}
                  ...
                  <div style={{ position: "absolute", inset: 0, display: "flex", alignItems: "center", justifyContent: "center", padding: 12 }}>
                    <div style={{ textAlign: "center", background: "var(--surface-card)", border: "1px solid var(--border)", borderRadius: "var(--radius-lg)", boxShadow: "var(--shadow-lg, 0 10px 30px -12px rgba(16,24,40,.25))", padding: "20px 24px", maxWidth: 420 }}>
                      <div style={{ display: "inline-flex", alignItems: "center", justifyContent: "center", width: 40, height: 40, borderRadius: 20, background: "linear-gradient(180deg,#F7CE63,#D99A1F)", color: "#4a3300", marginBottom: 8 }}>{I("lock", 18)}</div>
                      <div style={{ fontWeight: 700, color: "var(--text-strong)", fontSize: "var(--text-md)" }}>{locked} {locked === 1 ? T("more employer viewed you") : T("more employers viewed you")}</div>
                      <div style={{ fontSize: "var(--text-sm)", color: "var(--text-muted)", margin: "6px 0 14px", lineHeight: 1.5 }}>{T("Upgrade to Premium to see everyone who viewed your profile.")}</div>
                      <Button variant="primary" iconLeft={I("crown", 15)} onClick={function(){ setShowUpgrade(true); }}>{T("Upgrade to Premium")}</Button>
                    </div>
                  </div>
                </div>
              )}
