...
(function init() {
  ...
  const { Button, Badge, StatusBadge, Avatar, Card, StatCard, Tabs, EmptyState, Input, Textarea, Select: DSSelect, Switch } = NS;
  ...
  function MyProfile({ user, onUserUpdate }) {
    ...
    function deleteAcct() {
      ...
      emp.deleteAccount(delPwd).then((r) => {
        ...
        setDelMsg({ ok: true, text: (r && r.message) || "We've received your deletion request." });
      }).catch(e => { setDelBusy(false); setDelMsg({ ok: false, text: (e && e.message) || "Could not submit your request." }); });
    }
    function changePwd() {
    ...
    return (
      <div className="krm-page-pad" style={{ padding: 28, maxWidth: 720 }}>
        ...
        {/* Danger zone — request account & data deletion (satisfies app-store deletion policy). */}
        <Card padding={24} style={{ marginTop: 20, borderColor: "var(--danger)" }}>
          <h3 style={{ fontSize: "var(--text-base)", fontWeight: 700, color: "var(--danger)", marginBottom: 4 }}>Delete account</h3>
          <p style={{ fontSize: "var(--text-sm)", color: "var(--text-muted)", marginBottom: 16 }}>
            Request deletion of your account and personal data. Because an employer account is linked to your company, jobs and billing, our team reviews and completes the request within 30 days (some records may be retained where the law requires).
          </p>
          {delMsg && delMsg.ok ? (
            <div style={{ fontSize: "var(--text-sm)", color: "var(--success)", fontWeight: 600 }}>{delMsg.text}</div>
          ) : !delOpen ? (
            <Button variant="outline" style={{ color: "var(--danger)", borderColor: "var(--danger)" }} onClick={() => { setDelOpen(true); setDelMsg(null); }}>Request account deletion</Button>
          ) : (
            <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
              <Input label="Confirm your password" type="password" value={delPwd} onChange={e => setDelPwd(e.target.value)} placeholder="••••••••" />
              {delMsg && !delMsg.ok && <div style={{ fontSize: "var(--text-sm)", color: "var(--danger)", fontWeight: 600 }}>{delMsg.text}</div>}
              <div style={{ display: "flex", gap: 10 }}>
                <Button variant="primary" style={{ background: "var(--danger)", borderColor: "var(--danger)" }} disabled={delBusy || !delPwd} onClick={deleteAcct}>{delBusy ? "Submitting…" : "Submit request"}</Button>
                <Button variant="ghost" disabled={delBusy} onClick={() => { setDelOpen(false); setDelPwd(""); setDelMsg(null); }}>Cancel</Button>
              </div>
            </div>
          )}
        </Card>
  ...
  function Overview({ jobs, loading, onNav }) {
    ...
    React.useEffect(function () { emp.fetchUpcomingInterviews().then(function (d) { setUpcoming(d || []); }).catch(function () {}); }, []);
    const fmtDT = fmtWall;
    return (
      <div className="krm-page-pad" style={{ padding: 28, display: "flex", flexDirection: "column", gap: 24 }}>
  ...
  function TalentSearch({ jobs, onGoToMessages }) {
    ...
    const runSearch = function () {
      setLoading(true);
      emp.searchCandidates({ keyword: kw, skills: skills, languages: langs, per_page: 30 }).then(function (d) { setResults(d); setLoading(false); }).catch(function (e) { setLoading(false); flash((e && e.message) || "Search failed. Please try again."); });
    };
    React.useEffect(function () { runSearch(); }, []);
    const loadPool = React.useCallback(function () { emp.fetchTalentPool(poolKw).then(setPool).catch(function (e) { flash((e && e.message) || "Could not load the talent pool."); }); }, [poolKw]);
    React.useEffect(function () {
      if (tab !== "pool") return;
    ...
    const patchSaved = function (id, saved) {
      setResults(function (r) { return r ? Object.assign({}, r, { data: r.data.map(function (x) { return x.id === id ? Object.assign({}, x, { saved: saved }) : x; }) }) : r; });
      setDetail(function (d) { return d && d.id === id ? Object.assign({}, d, { saved: saved }) : d; });
      if (tab === "pool") loadPool();
    };
    const toggleSave = function (c) {
      if (c.saved) { emp.unsaveCandidate(c.id).then(function () { patchSaved(c.id, false); }).catch(function () {}); }
      else { emp.saveCandidate(c.id).then(function () { patchSaved(c.id, true); flash("Saved to talent pool."); }).catch(function () {}); }
    };
    const sendMessage = function () {
      if (!msgBody.trim() || !sel) return;
      emp.startConversation({ other_user_id: sel, message: msgBody.trim() })
        .then(function () { setComposing(false); setMsgBody(""); if (onGoToMessages) onGoToMessages(); })
        .catch(function (e) { flash("Error: " + (e && e.message)); });
    };
    const stripTags = function (s) { return String(s == null ? "" : s).replace(/<[^>]*>/g, ""); };
    const entryLine = function (e) {
      if (typeof e === "string") return e;
      if (e && typeof e === "object") { return Object.keys(e).filter(function (k) { return k !== "_k" && typeof e[k] !== "object" && String(e[k]).trim() !== ""; }).map(function (k) { return stripTags(e[k]); }).join(" · "); }
      return "";
    };
    const Chip = function (t) { return <span key={t} style={{ fontSize: 11, fontWeight: 600, color: "var(--text-brand)", background: "var(--brand-subtle)", borderRadius: 999, padding: "2px 9px" }}>{t}</span>; };

    const card = function (c) {
      return (
        <div key={c.id} onClick={function () { setSel(c.id); }} style={{ background: "var(--surface-card)", border: "1px solid var(--border)", borderRadius: "var(--radius-lg)", padding: 14, boxShadow: "var(--shadow-xs)", cursor: "pointer", display: "flex", flexDirection: "column", gap: 10 }}>
          <div style={{ display: "flex", alignItems: "center", gap: 11 }}>
            ...
            <div style={{ minWidth: 0, flex: 1 }}>
              ...
              <div style={{ fontSize: "var(--text-xs)", color: "var(--text-muted)", whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" }}>{c.headline || "—"}</div>
            </div>
          </div>
          {c.skills && c.skills.length > 0 && <div style={{ display: "flex", flexWrap: "wrap", gap: 5 }}>{c.skills.slice(0, 6).map(function (s) { return Chip(s); })}</div>}
          <div style={{ display: "flex", alignItems: "center", gap: 10, marginTop: 2 }}>
            {c.has_cv && <span style={{ fontSize: 11, color: "var(--text-faint)", display: "inline-flex", alignItems: "center", gap: 3 }}>{I("file-text", 11)} CV</span>}
            <div style={{ marginLeft: "auto" }} onClick={function (e) { e.stopPropagation(); toggleSave(c); }}>
              <Button variant={c.saved ? "secondary" : "ghost"} size="sm" iconLeft={I(c.saved ? "bookmark-check" : "bookmark", 13)}>{c.saved ? "Saved" : "Save"}</Button>
            </div>
          </div>
        </div>
      );
    ...
    return (
      <div className="krm-page-pad" style={{ padding: 28 }}>
        <div style={{ display: "flex", alignItems: "center", gap: 14, marginBottom: 18, flexWrap: "wrap" }}>
          <h1 style={{ fontSize: "var(--text-xl)", fontWeight: 700, color: "var(--text-strong)", margin: 0 }}>Find candidates</h1>
          <div style={{ display: "flex", gap: 4, background: "var(--surface-sunken)", borderRadius: "var(--radius-pill)", padding: 3 }}>
            {[{ id: "search", label: "Search" }, { id: "pool", label: "Talent pool" }].map(function (t) {
              return <button key={t.id} onClick={function () { setTab(t.id); }} style={{ border: "none", cursor: "pointer", borderRadius: "var(--radius-pill)", padding: "6px 16px", fontFamily: "var(--font-sans)", fontSize: "var(--text-sm)", fontWeight: 600, background: tab === t.id ? "var(--surface-card)" : "transparent", color: tab === t.id ? "var(--text-brand)" : "var(--text-muted)", boxShadow: tab === t.id ? "var(--shadow-xs)" : "none" }}>{t.label}</button>;
            })}
          </div>
        ...
        {tab === "search" ? (
          <div style={{ display: "flex", gap: 10, marginBottom: 18, flexWrap: "wrap", alignItems: "flex-end" }}>
            <div style={{ flex: 2, minWidth: 200 }}><Input label="Keyword" value={kw} onChange={function (e) { setKw(e.target.value); }} placeholder="name, title, summary…" onKeyDown={function (e) { if (e.key === "Enter") runSearch(); }} /></div>
            <div style={{ flex: 1, minWidth: 160 }}><Input label="Skills" value={skills} onChange={function (e) { setSkills(e.target.value); }} placeholder="React, SQL" onKeyDown={function (e) { if (e.key === "Enter") runSearch(); }} /></div>
            <div style={{ flex: 1, minWidth: 140 }}><Input label="Languages" value={langs} onChange={function (e) { setLangs(e.target.value); }} placeholder="English" onKeyDown={function (e) { if (e.key === "Enter") runSearch(); }} /></div>
            <Button variant="primary" iconLeft={I("search", 15)} onClick={runSearch}>Search</Button>
          </div>
        ) : (
          <div style={{ marginBottom: 18, maxWidth: 320 }}><Input value={poolKw} onChange={function (e) { setPoolKw(e.target.value); }} placeholder="Filter saved candidates…" iconLeft={I("search", 15)} /></div>
        )}
          ...
          (!listData || listData.length === 0)
            ...
            : (
              <div>
                ...
                <div className="krm-talent-grid" style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(240px, 1fr))", gap: 14 }}>
                  {listData.map(function (c) { return card(c); })}
                </div>
              </div>
            )
        )}
        ...
        {sel && (
          <div onClick={function () { setSel(null); }} style={{ position: "fixed", inset: 0, zIndex: 200, background: "var(--surface-overlay)", display: "flex", justifyContent: "flex-end" }}>
            <div onClick={function (e) { e.stopPropagation(); }} style={{ width: "100%", maxWidth: 460, height: "100%", background: "var(--surface-card)", boxShadow: "var(--shadow-xl)", display: "flex", flexDirection: "column", overflow: "hidden" }}>
              <div style={{ padding: "16px 20px", borderBottom: "1px solid var(--border)", display: "flex", alignItems: "center", gap: 12 }}>
                <Avatar src={detail && detail.avatar_url} name={(detail && detail.name) || "?"} size={44} />
                <div style={{ minWidth: 0, flex: 1 }}>
                  <div style={{ fontWeight: 700, color: "var(--text-strong)" }}>{detail ? detail.name : "Loading…"}</div>
                  <div style={{ fontSize: "var(--text-xs)", color: "var(--text-muted)" }}>{detail && detail.headline}</div>
                </div>
                  ...
                  <React.Fragment>
                    ...
                    {composing && (
                      <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
                        ...
                        <div><Button variant="primary" size="sm" disabled={!msgBody.trim()} onClick={sendMessage}>Send message</Button></div>
                      </div>
                    )}
                    {(detail.email || detail.phone) && (
                      <div style={{ display: "flex", flexDirection: "column", gap: 5, fontSize: "var(--text-sm)" }}>
                        {detail.email && <div style={{ display: "flex", alignItems: "center", gap: 8, color: "var(--text-body)" }}>{I("mail", 14)} <a href={"mailto:" + detail.email} style={{ color: "inherit", textDecoration: "none" }}>{detail.email}</a></div>}
                        {detail.phone && <div style={{ display: "flex", alignItems: "center", gap: 8, color: "var(--text-body)" }}>{I("phone", 14)} {detail.phone}</div>}
                      </div>
                    )}
                    {detail.summary && <div><div style={{ fontSize: "var(--text-xs)", fontWeight: 700, color: "var(--text-faint)", textTransform: "uppercase", letterSpacing: ".04em", marginBottom: 6 }}>Summary</div><div style={{ fontSize: "var(--text-sm)", color: "var(--text-body)", lineHeight: 1.6 }}>{stripTags(detail.summary)}</div></div>}
                    {detail.skills && detail.skills.length > 0 && <div><div style={{ fontSize: "var(--text-xs)", fontWeight: 700, color: "var(--text-faint)", textTransform: "uppercase", letterSpacing: ".04em", marginBottom: 8 }}>Skills</div><div style={{ display: "flex", flexWrap: "wrap", gap: 6 }}>{detail.skills.map(function (s) { return Chip(entryLine(s)); })}</div></div>}
                    {detail.experience && detail.experience.length > 0 && <div><div style={{ fontSize: "var(--text-xs)", fontWeight: 700, color: "var(--text-faint)", textTransform: "uppercase", letterSpacing: ".04em", marginBottom: 8 }}>Experience</div><div style={{ display: "flex", flexDirection: "column", gap: 6 }}>{detail.experience.map(function (e, i) { return <div key={i} style={{ fontSize: "var(--text-sm)", color: "var(--text-body)" }}>{entryLine(e)}</div>; })}</div></div>}
                    {detail.education && detail.education.length > 0 && <div><div style={{ fontSize: "var(--text-xs)", fontWeight: 700, color: "var(--text-faint)", textTransform: "uppercase", letterSpacing: ".04em", marginBottom: 8 }}>Education</div><div style={{ display: "flex", flexDirection: "column", gap: 6 }}>{detail.education.map(function (e, i) { return <div key={i} style={{ fontSize: "var(--text-sm)", color: "var(--text-body)" }}>{entryLine(e)}</div>; })}</div></div>}
                    {detail.languages && detail.languages.length > 0 && <div><div style={{ fontSize: "var(--text-xs)", fontWeight: 700, color: "var(--text-faint)", textTransform: "uppercase", letterSpacing: ".04em", marginBottom: 8 }}>Languages</div><div style={{ display: "flex", flexWrap: "wrap", gap: 6 }}>{detail.languages.map(function (l) { return Chip(entryLine(l)); })}</div></div>}
                    {detail.certifications && detail.certifications.length > 0 && <div><div style={{ fontSize: "var(--text-xs)", fontWeight: 700, color: "var(--text-faint)", textTransform: "uppercase", letterSpacing: ".04em", marginBottom: 8 }}>Certifications</div><div style={{ display: "flex", flexWrap: "wrap", gap: 6 }}>{detail.certifications.map(function (c) { return Chip(entryLine(c)); })}</div></div>}
                  </React.Fragment>
                )}
              </div>
            </div>
  ...
  const IV_TYPE = { phone: "Phone", video: "Video", in_person: "In-person" };
  const IV_STATUS = [
    { value: "scheduled", label: "Scheduled" }, { value: "confirmed", label: "Confirmed" }, { value: "rescheduled", label: "Rescheduled" },
    { value: "completed", label: "Completed" }, { value: "cancelled", label: "Cancelled" }, { value: "no_show", label: "No show" },
  ];
  const SC_CRITERIA = [
    { key: "technical", label: "Technical skills" }, { key: "communication", label: "Communication" }, { key: "experience", label: "Experience" },
    { key: "problem_solving", label: "Problem solving" }, { key: "culture_fit", label: "Culture fit" },
  ];
  const SC_REC = [{ value: "", label: "— Recommendation —" }, { value: "strong_hire", label: "Strong hire" }, { value: "hire", label: "Hire" }, { value: "maybe", label: "Maybe" }, { value: "no_hire", label: "No hire" }];
  const REC_LABEL = { strong_hire: "Strong hire", hire: "Hire", maybe: "Maybe", no_hire: "No hire" };
  const ivInput = { width: "100%", boxSizing: "border-box", border: "1px solid var(--border)", borderRadius: "var(--radius-sm)", padding: "7px 10px", fontFamily: "var(--font-sans)", fontSize: "var(--text-sm)", color: "var(--text-body)", background: "var(--surface-card)", outline: "none" };
  ...
  function ScorecardEditor({ iv, onSaved, flash }) {
    var mine = (iv.scorecards || []).find(function (s) { return s.can_edit; });
    ...
    const [busy, setBusy] = React.useState(false);
    var others = (iv.scorecards || []).filter(function (s) { return !s.can_edit; });
    const save = function () {
      setBusy(true);
    ...
    return (
      <div style={{ marginTop: 8, paddingTop: 8, borderTop: "1px dashed var(--border)" }}>
        {others.map(function (s) { return <div key={s.id} style={{ fontSize: 11, color: "var(--text-muted)", marginBottom: 4 }}>{s.author}: <b>{REC_LABEL[s.recommendation] || "—"}</b></div>; })}
        <div style={{ display: "flex", flexDirection: "column", gap: 5 }}>
          {SC_CRITERIA.map(function (c) {
            return (
              <div key={c.key} style={{ display: "flex", alignItems: "center", gap: 8 }}>
                <span style={{ flex: 1, fontSize: 12, color: "var(--text-body)" }}>{c.label}</span>
                <div style={{ display: "flex", gap: 3 }}>
                  {[1, 2, 3, 4, 5].map(function (n) {
                    var on = (ratings[c.key] || 0) >= n;
                    return <span key={n} onClick={function () { setRatings(Object.assign({}, ratings, { [c.key]: n })); }} style={{ cursor: "pointer", color: on ? "var(--warning)" : "var(--border-strong)", fontSize: 16, lineHeight: 1 }}>★</span>;
                  })}
                </div>
              </div>
            );
  ...
  function InterviewsPanel({ appId, flash }) {
    const [list, setList] = React.useState([]);
    const [loading, setLoading] = React.useState(true);
    const [open, setOpen] = React.useState(false);
    const [scOpen, setScOpen] = React.useState(null);
    const IVBLANK = { type: "video", scheduled_at: "", duration_min: 45, timezone: "Asia/Phnom_Penh", meeting_url: "", location: "", notes: "" };
          ...
          list.length === 0 ? <div style={{ fontSize: "var(--text-xs)", color: "var(--text-faint)" }}>{T("No interviews scheduled.")}</div> : (
            <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
              {list.map(function (iv) {
                return (
                  <div key={iv.id} style={{ background: "var(--surface-sunken)", borderRadius: "var(--radius-md)", padding: "10px 12px" }}>
                    <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                      <span style={{ display: "inline-flex", color: "var(--text-brand)" }}>{I(iv.type === "phone" ? "phone" : iv.type === "in_person" ? "map-pin" : "video", 14)}</span>
                      <div style={{ flex: 1, minWidth: 0 }}>
                        <div style={{ fontSize: "var(--text-sm)", fontWeight: 600, color: "var(--text-strong)" }}>{fmt(iv.scheduled_at)}</div>
  ...
  function CreateCompanyForm({ onCreated }) {
    ...
    React.useEffect(function () {
      emp.myCompanyClaims().then(function (d) {
        var p = (d.data || []).filter(function (x) { return x.status === "pending"; })[0];
        if (p) setMyClaim(p);
      }).catch(function () {});
    }, []);
    const onSearch = function (v) {
      setCq(v); setCMsg(null);
      if (cTimer.current) clearTimeout(cTimer.current);
      if (v.trim().length < 2) { setCResults([]); return; }
      setCSearching(true);
      cTimer.current = setTimeout(function () {
        emp.searchClaimable(v.trim()).then(function (d) { setCResults(d.data || []); setCSearching(false); })
          .catch(function () { setCSearching(false); });
      }, 350);
    };
    const requestClaim = function (co) {
      ...
      emp.requestCompanyClaim(co.id).then(function () {
        setMyClaim({ status: "pending", company: { id: co.id, name: co.name } });
        setCResults([]); setCq("");
      }).catch(function (e) { setCMsg({ ok: false, text: (e && e.message) || "Could not submit the request." }); });
    };
    const submit = () => {
    ...
    return (
      <div className="krm-page-pad" style={{ padding: 28 }}>
        <div style={{ maxWidth: 620 }}>
          ...
          {/* Claim a company we already set up on your behalf. */}
          <div style={{ display: "flex", alignItems: "center", gap: 12, margin: "24px 0 16px" }}>
            <div style={{ flex: 1, height: 1, background: "var(--border)" }} />
            <span style={{ fontSize: "var(--text-xs)", fontWeight: 700, color: "var(--text-muted)", textTransform: "uppercase", letterSpacing: ".05em" }}>or</span>
            <div style={{ flex: 1, height: 1, background: "var(--border)" }} />
          </div>
          <Card padding={24}>
            <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 4 }}>
              <span style={{ display: "inline-flex", alignItems: "center", justifyContent: "center", width: 34, height: 34, borderRadius: 8, background: "var(--brand-subtle)", color: "var(--brand)" }}>{I("building-2", 18)}</span>
              <h3 style={{ fontSize: "var(--text-base)", fontWeight: 700, color: "var(--text-strong)", margin: 0 }}>Already have a company on Krama?</h3>
            </div>
            <p style={{ fontSize: "var(--text-sm)", color: "var(--text-muted)", margin: "0 0 16px" }}>If we set up your company for you, search for it and request access. An admin will verify and transfer ownership to you.</p>

            {myClaim ? (
              <div style={{ display: "flex", alignItems: "center", gap: 10, padding: "12px 14px", borderRadius: "var(--radius-md)", background: "var(--warning-bg, #fff8ec)", border: "1px solid var(--warning-border, #f0d9a8)" }}>
                {I("clock", 16)}
                <div style={{ fontSize: "var(--text-sm)", color: "var(--text-strong)", fontWeight: 600 }}>Request pending for “{myClaim.company && myClaim.company.name}” — we'll grant access once it's reviewed.</div>
              </div>
            ) : (
              <React.Fragment>
                <Input value={cq} onChange={(e) => onSearch(e.target.value)} placeholder="Search your company name…" iconLeft={I("search", 16)} />
                {cMsg && !cMsg.ok && <div style={{ color: "var(--danger)", fontSize: "var(--text-sm)", fontWeight: 600, marginTop: 8 }}>{cMsg.text}</div>}
                <div style={{ marginTop: 10, display: "flex", flexDirection: "column", gap: 8 }}>
                  {cSearching && <div style={{ fontSize: "var(--text-sm)", color: "var(--text-muted)" }}>Searching…</div>}
                  {!cSearching && cq.trim().length >= 2 && cResults.length === 0 && <div style={{ fontSize: "var(--text-sm)", color: "var(--text-muted)" }}>No matching company found.</div>}
                  {cResults.map(function (co) {
                    return (
                      <div key={co.id} style={{ display: "flex", alignItems: "center", justifyContent: "space-between", gap: 12, padding: "10px 12px", border: "1px solid var(--border)", borderRadius: "var(--radius-md)" }}>
                        <div style={{ display: "flex", alignItems: "center", gap: 10, minWidth: 0 }}>
                          {co.logo_url ? <img src={co.logo_url} alt="" width="32" height="32" style={{ borderRadius: 7, objectFit: "cover", flexShrink: 0 }} /> : <span style={{ width: 32, height: 32, borderRadius: 7, background: "var(--surface-sunken, var(--surface-page))", display: "inline-flex", alignItems: "center", justifyContent: "center", flexShrink: 0, color: "var(--text-muted)" }}>{I("building-2", 15)}</span>}
                          <span style={{ fontWeight: 600, color: "var(--text-strong)", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{co.name}</span>
                        </div>
                        <Button variant="outline" size="sm" style={{ flexShrink: 0 }} onClick={function () { requestClaim(co); }}>Request access</Button>
                      </div>
                    );
                  })}
                </div>
