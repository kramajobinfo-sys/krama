      ...
      <div className="krm-page-pad" style={{ padding: 28, maxWidth: 1100 }}>
        ...
        <Card padding={24} style={{ marginBottom: 18 }}>
          ...
          <div style={{ marginTop: 18, paddingTop: 16, borderTop: "1px solid var(--border-subtle)", opacity: (s.premiumFeaturedVisible !== false) ? 1 : 0.45, pointerEvents: (s.premiumFeaturedVisible !== false) ? "auto" : "none" }}>
            <div className="krm-form-grid" style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16, marginBottom: 18 }}>
              <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", gap: 12, padding: "6px 0" }}>
                <div>
                  <div style={{ fontSize: "var(--text-sm)", fontWeight: 700, color: "var(--text-strong)" }}>Slot limit</div>
                  <div style={{ fontSize: "var(--text-xs)", color: "var(--text-muted)", marginTop: 2 }}>Max premium companies</div>
                </div>
                <Stepper value={s.premiumFeaturedLimit || 10} min={1} max={50} onChange={(v) => set("premiumFeaturedLimit", v)} />
              </div>
              <Input label="Price per slot" value={s.premiumFeaturedPrice || ""} onChange={(e) => set("premiumFeaturedPrice", e.target.value)} placeholder="e.g. $99 / month" hint="Shown to employers once subscriptions launch." />
            </div>
