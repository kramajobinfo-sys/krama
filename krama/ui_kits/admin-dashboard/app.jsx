...
(function init() {
  ...
  const { Button, Badge, StatusBadge, Avatar, Card, StatCard, Tabs, Checkbox, Switch, Select: DSSelect, Input, Textarea, IconButton, EmptyState } = NS;
  ...
  function normalizeOptions(options, children) {
    if (options && options.length) {
      return options.map(function (o) { return typeof o === "string" ? { value: o, label: o } : o; });
    }
    var out = [];
    React.Children.forEach(children, function (c) {
      if (c && c.type === "option") out.push({ value: c.props.value, label: c.props.children });
    });
    return out;
  }
  ...
  function MobileSelect(props) {
    ...
    var placeholder = props.placeholder, disabled = props.disabled;
    var hint = props.hint, error = props.error;
    var opts = normalizeOptions(props.options, props.children);
    var btnRef = React.useRef(null);
    var listRef = React.useRef(null);
    var _o = React.useState(false), open = _o[0], setOpen = _o[1];
    var _r = React.useState(null), pos = _r[0], setPos = _r[1];

    var selected = opts.find(function (o) { return String(o.value) === String(value); });
    var display = selected ? selected.label : (placeholder || "");

    var place = function () {
      var el = btnRef.current; if (!el) return;
      var r = el.getBoundingClientRect();
      var below = window.innerHeight - r.bottom, above = r.top;
      var openUp = below < 240 && above > below;
      setPos({ left: r.left, width: r.width, top: openUp ? null : r.bottom + 4, bottom: openUp ? (window.innerHeight - r.top + 4) : null, maxH: Math.max(160, (openUp ? above : below) - 16) });
    };
    var doOpen = function () { if (disabled) return; place(); setOpen(true); };
    var pick = function (o) { setOpen(false); if (onChange) onChange({ target: { value: o.value } }); };
    ...
    React.useEffect(function () {
      ...
      var onResize = function () { setOpen(false); };
    ...
    return React.createElement("div", { style: Object.assign({ display: "flex", flexDirection: "column", gap: 6 }, props.containerStyle) },
      label && React.createElement("label", { style: { fontSize: "var(--text-sm)", fontWeight: 600, color: "var(--text-strong)" } }, label),
      React.createElement("div", { style: { position: "relative" } },
        React.createElement("button", {
          ref: btnRef, type: "button", disabled: disabled, onClick: doOpen,
          "aria-haspopup": "listbox", "aria-expanded": open,
          style: Object.assign({
            appearance: "none", width: "100%", height: 44, padding: "0 38px 0 14px", textAlign: "left",
        ...
        React.createElement("span", { style: { position: "absolute", right: 14, top: "50%", transform: "translateY(-50%)", pointerEvents: "none", color: "var(--text-faint)" } },
      ...
      (hint || error) && React.createElement("span", { style: { fontSize: "var(--text-xs)", color: error ? "var(--danger)" : "var(--text-muted)" } }, error || hint),
      open && pos && window.ReactDOM.createPortal(
        React.createElement("div", { onClick: function () { setOpen(false); }, style: { position: "fixed", inset: 0, zIndex: 4000 } },
          React.createElement("div", {
            ref: listRef, role: "listbox", onClick: function (e) { e.stopPropagation(); },
            style: {
              ...
              borderRadius: "var(--radius-md)", boxShadow: "var(--shadow-xl)", padding: 4,
            },
          }, opts.map(function (o, i) {
            var isSel = String(o.value) === String(value);
            return React.createElement("div", {
              key: String(o.value) + i, role: "option", "aria-selected": isSel, onClick: function () { pick(o); },
              style: {
                display: "flex", alignItems: "center", justifyContent: "space-between", gap: 8,
                padding: "9px 12px", borderRadius: "var(--radius-sm, 6px)", cursor: "pointer",
                fontSize: "var(--text-sm)", lineHeight: 1.4,
                color: isSel ? "var(--text-brand)" : "var(--text-body)",
                fontWeight: isSel ? 600 : 400, background: isSel ? "var(--brand-subtle)" : "transparent",
              },
            }, React.createElement("span", null, o.label),
               isSel && React.createElement("span", { style: { flexShrink: 0, display: "inline-flex", color: "var(--text-brand)" } },
                 React.createElement("svg", { width: "15", height: "15", viewBox: "0 0 24 24", fill: "none", stroke: "currentColor", strokeWidth: "2.4", strokeLinecap: "round", strokeLinejoin: "round" },
  ...
  function Homepage() {
    ...
    const togglePremiumFeatured = (name) => { setS((x) => { const list = x.premiumFeatured || []; const has = list.includes(name); if (!has && list.length >= (x.premiumFeaturedLimit || 10)) return x; return { ...x, premiumFeatured: has ? list.filter((n) => n !== name) : [...list, name] }; }); setSaved(false); };
    const apply = () => {
      setSaveErr("");
    ...
    // Large 1600 x 480 page-hero editor (heading + subtitle + background image),
    // reused for the info pages (Pricing / About / Contact / Terms / Privacy).
    const HeroCard = (key, label, desc, defaultHeading) => {
      const h = s[key] || {};
      const img = h.image || "";
      return (
        <Card padding={24} style={{ marginTop: 18 }}>
          ...
          <div style={{ marginTop: 16, paddingTop: 14, borderTop: "1px solid var(--border-subtle)", display: "flex", alignItems: "center", gap: 12 }}>
            <div style={{ width: 88, height: 52, flexShrink: 0, borderRadius: "var(--radius-md)", border: "1px solid var(--border)", background: img ? "transparent" : "var(--surface-sunken)", backgroundImage: img ? "url('" + img + "')" : "none", backgroundSize: "cover", backgroundPosition: "center", display: "flex", alignItems: "center", justifyContent: "center" }}>
              {h._uploading && <span style={{ fontSize: 10, color: "var(--text-muted)", fontFamily: "var(--font-sans)" }}>Uploading…</span>}
            </div>
            <div style={{ flex: 1 }}>
              <label style={{ display: "inline-flex", alignItems: "center", gap: 6, cursor: h._uploading ? "not-allowed" : "pointer", opacity: h._uploading ? 0.5 : 1, fontFamily: "var(--font-sans)", fontSize: "var(--text-sm)", fontWeight: 700, color: "var(--text-brand)" }}>
                {I("image", 14)} {h._uploading ? "Uploading…" : img ? "Replace background image" : "Upload background image"}
                <input type="file" accept="image/*" disabled={!!h._uploading} onChange={uploadTo(key)} style={{ display: "none" }} />
              </label>
              {img ? (
                <div>
                  <div style={{ display: "flex", gap: 6, marginTop: 8 }}>
                    {["cover", "contain"].map((f) => (
                      <button key={f} onClick={() => setBanner(key, "fit", f)} style={{ height: 28, padding: "0 12px", borderRadius: "var(--radius-sm)", cursor: "pointer", border: "1px solid " + ((h.fit || "cover") === f ? "var(--brand)" : "var(--border-strong)"), background: (h.fit || "cover") === f ? "var(--brand-subtle)" : "var(--surface-card)", color: (h.fit || "cover") === f ? "var(--text-brand)" : "var(--text-muted)", fontFamily: "var(--font-sans)", fontSize: "var(--text-xs)", fontWeight: 700 }}>{f}</button>
                    ))}
                    <button onClick={() => setBanner(key, "image", "")} style={{ height: 28, padding: "0 12px", borderRadius: "var(--radius-sm)", cursor: "pointer", border: "1px solid var(--border-strong)", background: "var(--surface-card)", color: "var(--text-muted)", fontFamily: "var(--font-sans)", fontSize: "var(--text-xs)", fontWeight: 700 }}>Remove</button>
                  </div>
                  <div style={{ marginTop: 12 }}>
                    <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 4 }}>
                      <span style={{ fontSize: "var(--text-xs)", fontWeight: 600, color: "var(--text-muted)" }}>Image visibility</span>
                      <span style={{ fontSize: "var(--text-xs)", fontWeight: 700, color: "var(--text-brand)" }}>{100 - (h.imgOverlay != null ? h.imgOverlay : 45)}%</span>
                    </div>
                    <input type="range" min={0} max={90} step={5} value={h.imgOverlay != null ? h.imgOverlay : 45} onChange={(e) => setBanner(key, "imgOverlay", Number(e.target.value))} style={{ width: "100%", accentColor: "var(--brand)" }} />
                    <div style={{ display: "flex", justifyContent: "space-between", fontSize: 10, color: "var(--text-muted)", marginTop: 2 }}><span>Full colour</span><span>Clear image</span></div>
                  </div>
                </div>
          ...
          <div style={{ marginTop: 14, borderRadius: "var(--radius-md)", overflow: "hidden", border: "1px solid var(--border)" }}>
            ...
            <div style={{ position: "relative", background: "var(--teal-800)", color: "#fff", padding: "20px 24px", overflow: "hidden" }}>
              {img && <React.Fragment><div style={{ position: "absolute", inset: 0, backgroundImage: "url('" + img + "')", backgroundSize: (h.fit || "cover"), backgroundPosition: "center" }} /><div style={{ position: "absolute", inset: 0, background: "var(--teal-800)", opacity: (h.imgOverlay != null ? h.imgOverlay : 45) / 100 }} /></React.Fragment>}
              <div style={{ position: "relative" }}>
                <div style={{ fontSize: 22, fontWeight: 800 }}>{h.heading != null ? h.heading : defaultHeading}</div>
                <div style={{ fontSize: 14, color: "var(--stone-300)", marginTop: 6 }}>{h.sub || ""}</div>
              </div>
            </div>
          </div>
        </Card>
    ...
    return (
      <div className="krm-page-pad" style={{ padding: 28, maxWidth: 1100 }}>
        ...
        {hpTab === "pages" && <React.Fragment>
          ...
          {HeroCard("termsHero", "Terms -- Hero banner", "The hero at the top of the Terms of Service page.", "Terms of Service")}
          {HeroCard("privacyHero", "Privacy -- Hero banner", "The hero at the top of the Privacy Policy page.", "Privacy Policy")}
        </React.Fragment>}
        ...
        <Card padding={24} style={{ marginBottom: 18 }}>
          ...
          <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginTop: 14, paddingTop: 14, borderTop: "1px solid var(--border-subtle)", opacity: s.topVisible ? 1 : 0.45, pointerEvents: s.topVisible ? "auto" : "none" }}>
            <div>
              <div style={{ fontSize: "var(--text-sm)", fontWeight: 700, color: "var(--text-strong)" }}>Only employers with open jobs</div>
              <div style={{ fontSize: "var(--text-xs)", color: "var(--text-muted)", marginTop: 2 }}>Hide companies that currently have no published jobs from the carousel.</div>
            </div>
            <Switch checked={s.topOnlyWithJobs !== false} onChange={(v) => set("topOnlyWithJobs", v)} />
          </div>
        </Card>
        ...
        {(function() {
          const cmh = s.communityHero || {};
          const cmImg = cmh.image || "";
          return (
          <Card padding={24} style={{ marginTop: 18 }}>
            <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 16 }}>
              <span style={{ display: "inline-flex", alignItems: "center", justifyContent: "center", width: 36, height: 36, borderRadius: "var(--radius-md)", background: "var(--teal-50)", color: "var(--teal-700)" }}>{I("layout-panel-top", 18)}</span>
            ...
            {/* Background image */}
            <div style={{ marginTop: 16, paddingTop: 14, borderTop: "1px solid var(--border-subtle)", display: "flex", alignItems: "center", gap: 12 }}>
              <div style={{ width: 88, height: 52, flexShrink: 0, borderRadius: "var(--radius-md)", border: "1px solid var(--border)", background: cmImg ? "transparent" : "var(--surface-sunken)", backgroundImage: cmImg ? "url('" + cmImg + "')" : "none", backgroundSize: "cover", backgroundPosition: "center", display: "flex", alignItems: "center", justifyContent: "center" }}>
                {cmh._uploading && <span style={{ fontSize: 10, color: "var(--text-muted)", fontFamily: "var(--font-sans)" }}>Uploading…</span>}
              </div>
              <div style={{ flex: 1 }}>
                <label style={{ display: "inline-flex", alignItems: "center", gap: 6, cursor: cmh._uploading ? "not-allowed" : "pointer", opacity: cmh._uploading ? 0.5 : 1, fontFamily: "var(--font-sans)", fontSize: "var(--text-sm)", fontWeight: 700, color: "var(--text-brand)" }}>
                  {I("image", 14)} {cmh._uploading ? "Uploading…" : cmImg ? "Replace background image" : "Upload background image"}
                  <input type="file" accept="image/*" disabled={!!cmh._uploading} onChange={uploadTo("communityHero")} style={{ display: "none" }} />
                </label>
                {cmImg ? (
                  <div>
                    <div style={{ display: "flex", gap: 6, marginTop: 8 }}>
                      {["cover", "contain"].map((f) => (
                        <button key={f} onClick={() => setBanner("communityHero", "fit", f)} style={{ height: 28, padding: "0 12px", borderRadius: "var(--radius-sm)", cursor: "pointer", border: "1px solid " + ((cmh.fit || "cover") === f ? "var(--brand)" : "var(--border-strong)"), background: (cmh.fit || "cover") === f ? "var(--brand-subtle)" : "var(--surface-card)", color: (cmh.fit || "cover") === f ? "var(--text-brand)" : "var(--text-muted)", fontFamily: "var(--font-sans)", fontSize: "var(--text-xs)", fontWeight: 700 }}>{f}</button>
                      ))}
                      <button onClick={() => setBanner("communityHero", "image", "")} style={{ height: 28, padding: "0 12px", borderRadius: "var(--radius-sm)", cursor: "pointer", border: "1px solid var(--border-strong)", background: "var(--surface-card)", color: "var(--text-muted)", fontFamily: "var(--font-sans)", fontSize: "var(--text-xs)", fontWeight: 700 }}>Remove</button>
                    </div>
                    <div style={{ marginTop: 12 }}>
                      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 4 }}>
                        <span style={{ fontSize: "var(--text-xs)", fontWeight: 600, color: "var(--text-muted)" }}>Image visibility</span>
                        <span style={{ fontSize: "var(--text-xs)", fontWeight: 700, color: "var(--text-brand)" }}>{100 - (cmh.imgOverlay != null ? cmh.imgOverlay : 45)}%</span>
                      </div>
                      <input type="range" min={0} max={90} step={5} value={cmh.imgOverlay != null ? cmh.imgOverlay : 45} onChange={(e) => setBanner("communityHero", "imgOverlay", Number(e.target.value))} style={{ width: "100%", accentColor: "var(--brand)" }} />
                      <div style={{ display: "flex", justifyContent: "space-between", fontSize: 10, color: "var(--text-muted)", marginTop: 2 }}><span>Full colour</span><span>Clear image</span></div>
                    </div>
                  </div>
                ) : <div style={{ fontSize: "var(--text-xs)", color: "var(--text-muted)", marginTop: 4 }}>Optional — sits behind the heading text.</div>}
            ...
            <div style={{ marginTop: 14, borderRadius: "var(--radius-md)", overflow: "hidden", border: "1px solid var(--border)" }}>
              <div style={{ padding: "5px 10px", background: "var(--surface-sunken)", fontSize: "var(--text-xs)", color: "var(--text-faint)", fontWeight: 700, textTransform: "uppercase", letterSpacing: ".05em" }}>Preview</div>
              <div style={{ position: "relative", background: "var(--teal-800)", color: "#fff", padding: "20px 24px", overflow: "hidden" }}>
                {cmImg && <React.Fragment><div style={{ position: "absolute", inset: 0, backgroundImage: "url('" + cmImg + "')", backgroundSize: (cmh.fit || "cover"), backgroundPosition: "center" }} /><div style={{ position: "absolute", inset: 0, background: "var(--teal-800)", opacity: (cmh.imgOverlay != null ? cmh.imgOverlay : 45) / 100 }} /></React.Fragment>}
                <div style={{ position: "relative" }}>
                  <div style={{ fontSize: 22, fontWeight: 800 }}>{cmh.heading || "Connect with the Krama community"}</div>
                  <div style={{ fontSize: 14, color: "var(--stone-300)", marginTop: 6 }}>{cmh.sub || ""}</div>
                </div>
              </div>
            </div>
          </Card>
        ...
        <Card padding={24} style={{ marginBottom: 18 }}>
          <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between", gap: 16 }}>
            <div>
              <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                <span style={{ display: "inline-flex", alignItems: "center", justifyContent: "center", width: 36, height: 36, borderRadius: "var(--radius-md)", background: "linear-gradient(180deg,#F6C556,#D99A1F)", color: "#4a3600" }}>{I("star", 18)}</span>
                <h3 style={{ fontSize: "var(--text-lg)", fontWeight: 700, color: "var(--text-strong)" }}>Premium Featured companies</h3>
              </div>
              <p style={{ fontSize: "var(--text-sm)", color: "var(--text-muted)", marginTop: 8 }}>A paid, limited tier shown <strong>above</strong> the regular Featured companies with a gold highlight. Employer subscriptions aren&rsquo;t live yet, so pick premium companies manually below (up to the slot limit).</p>
            </div>
            <Switch checked={s.premiumFeaturedVisible !== false} onChange={(v) => set("premiumFeaturedVisible", v)} />
          </div>
          <div style={{ marginTop: 18, paddingTop: 16, borderTop: "1px solid var(--border-subtle)", opacity: (s.premiumFeaturedVisible !== false) ? 1 : 0.45, pointerEvents: (s.premiumFeaturedVisible !== false) ? "auto" : "none" }}>
            <div style={{ display: "grid", gap: 14, marginBottom: 18 }}>
              <div className="krm-form-grid" style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 12, alignItems: "end" }}>
                <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
                  <span style={{ fontSize: "var(--text-sm)", fontWeight: 700, color: "var(--text-strong)" }}>Slot limit</span>
                  <Stepper value={s.premiumFeaturedLimit || 10} min={1} max={50} onChange={(v) => set("premiumFeaturedLimit", v)} />
                  <span style={{ fontSize: "var(--text-xs)", color: "var(--text-muted)" }}>Max slots · desktop show count (4 cols)</span>
                </div>
                <Input label="Price / slot" type="number" min="0" step="1" value={s.premiumSlotPrice != null ? s.premiumSlotPrice : 49} onChange={(e) => set("premiumSlotPrice", e.target.value === "" ? "" : Number(e.target.value))} placeholder="49" />
                <Select label="Currency" value={s.premiumSlotCurrency || "USD"} onChange={(e) => set("premiumSlotCurrency", e.target.value)} options={[{ value: "USD", label: "USD ($)" }, { value: "KHR", label: "KHR (៛)" }]} />
              </div>
              <div className="krm-form-grid" style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
                <Input label="Duration (days)" type="number" min="1" step="1" value={s.premiumSlotDays != null ? s.premiumSlotDays : 30} onChange={(e) => set("premiumSlotDays", e.target.value === "" ? "" : Number(e.target.value))} placeholder="30" hint="One monthly period. Employers renew to stay premium." />
                <Input label="Annual price (0 = off)" type="number" min="0" step="1" value={s.premiumSlotAnnualPrice != null ? s.premiumSlotAnnualPrice : 0} onChange={(e) => set("premiumSlotAnnualPrice", e.target.value === "" ? "" : Number(e.target.value))} placeholder="0" hint="If set, employers can pay yearly (365 days) at this price instead of monthly." />
              </div>
              <div className="krm-form-grid" style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12, alignItems: "end" }}>
                <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
                  <span style={{ fontSize: "var(--text-sm)", fontWeight: 700, color: "var(--text-strong)" }}>Show on mobile</span>
                  <Stepper value={s.premiumFeaturedCountMobile || s.premiumFeaturedLimit || 10} min={1} max={50} onChange={(v) => set("premiumFeaturedCountMobile", v)} />
                  <span style={{ fontSize: "var(--text-xs)", color: "var(--text-muted)" }}>Phones show 3 columns · e.g. 9 = 3 full rows (capped at the slot limit)</span>
                </div>
              </div>
            </div>
            {premiumOverview && (
            ...
            <div className="krm-form-grid" style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
              {companies.map((name) => {
                const on = (s.premiumFeatured || []).includes(name);
                const atCap = (s.premiumFeatured || []).length >= (s.premiumFeaturedLimit || 10);
                const disabled = !on && atCap;
                return (
                  <button key={name} onClick={() => togglePremiumFeatured(name)} disabled={disabled} title={disabled ? "Slot limit reached" : undefined} style={{
                    display: "flex", alignItems: "center", gap: 12, padding: "12px 14px", cursor: disabled ? "not-allowed" : "pointer", textAlign: "left", opacity: disabled ? 0.5 : 1,
                    border: "1px solid " + (on ? "#D9A521" : "var(--border-strong)"),
                    background: on ? "rgba(246,197,86,0.14)" : "var(--surface-card)", borderRadius: "var(--radius-md)",
                    boxShadow: on ? "0 0 0 1px #D9A521" : "none",
                  }}>
                    ...
                    <span style={{ display: "inline-flex", alignItems: "center", justifyContent: "center", width: 20, height: 20, borderRadius: "var(--radius-sm)", border: "1.5px solid " + (on ? "#D9A521" : "var(--border-strong)"), background: on ? "#D9A521" : "transparent", color: "#fff" }}>{on ? I("check", 13) : null}</span>
                  </button>
                );
              })}
            </div>
        ...
        <Card padding={24}>
          ...
          <div style={{ marginTop: 20, paddingTop: 18, borderTop: "1px solid var(--border-subtle)", opacity: s.featuredVisible ? 1 : 0.45, pointerEvents: s.featuredVisible ? "auto" : "none" }}>
            <div className="krm-form-grid" style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12, marginBottom: 18 }}>
              <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
                <span style={{ fontSize: "var(--text-sm)", fontWeight: 700, color: "var(--text-strong)" }}>Per page — desktop</span>
                <Stepper value={s.featuredCount || 8} min={1} max={40} onChange={(v) => set("featuredCount", v)} />
                <span style={{ fontSize: "var(--text-xs)", color: "var(--text-muted)" }}>4 columns · 8 = 2 full rows</span>
              </div>
              <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
                <span style={{ fontSize: "var(--text-sm)", fontWeight: 700, color: "var(--text-strong)" }}>Per page — mobile</span>
                <Stepper value={s.featuredCountMobile || 9} min={1} max={40} onChange={(v) => set("featuredCountMobile", v)} />
                <span style={{ fontSize: "var(--text-xs)", color: "var(--text-muted)" }}>3 columns · 9 = 3 full rows</span>
              </div>
            </div>
            <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 14 }}>
        ...
        {/* Companies directory — how many company cards show per page */}
        <Card padding={24}>
          <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 6 }}>
            <span style={{ display: "inline-flex", alignItems: "center", justifyContent: "center", width: 36, height: 36, borderRadius: "var(--radius-md)", background: "var(--brand-subtle)", color: "var(--brand)" }}>{I("building-2", 18)}</span>
            <h3 style={{ fontSize: "var(--text-lg)", fontWeight: 700, color: "var(--text-strong)" }}>Companies directory — per page</h3>
          </div>
          <p style={{ fontSize: "var(--text-sm)", color: "var(--text-muted)", marginBottom: 18 }}>How many company cards show per page in the grid view of the public Companies page. Separate counts per device so each grid tiles evenly.</p>
          <div className="krm-form-grid" style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
            <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
              <span style={{ fontSize: "var(--text-sm)", fontWeight: 700, color: "var(--text-strong)" }}>Per page — desktop</span>
              <Stepper value={s.companiesPerPage || 12} min={1} max={48} onChange={(v) => set("companiesPerPage", v)} />
              <span style={{ fontSize: "var(--text-xs)", color: "var(--text-muted)" }}>Grid of 3 · 12 = 4 full rows</span>
            </div>
            <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
              <span style={{ fontSize: "var(--text-sm)", fontWeight: 700, color: "var(--text-strong)" }}>Per page — mobile</span>
              <Stepper value={s.companiesPerPageMobile || 9} min={1} max={48} onChange={(v) => set("companiesPerPageMobile", v)} />
              <span style={{ fontSize: "var(--text-xs)", color: "var(--text-muted)" }}>3 columns · 9 = 3 full rows</span>
            </div>
          </div>
        </Card>
        ...
        {/* Company Profile -- Default hero banner (fallback when an employer hasn't uploaded a cover) */}
        {(function() {
          const cph = s.companyProfileHero || {};
          const cphImg = cph.image || "";
          return (
          <Card padding={24} style={{ marginTop: 18 }}>
            <div style={{ display: "flex", alignItems: "flex-start", gap: 10, marginBottom: 8 }}>
              <span style={{ display: "inline-flex", alignItems: "center", justifyContent: "center", width: 36, height: 36, flexShrink: 0, borderRadius: "var(--radius-md)", background: "var(--teal-50)", color: "var(--teal-700)" }}>{I("image", 18)}</span>
              <div>
                <h3 style={{ fontSize: "var(--text-lg)", fontWeight: 700, color: "var(--text-strong)" }}>Company Profile -- Default hero banner</h3>
                <p style={{ fontSize: "var(--text-sm)", color: "var(--text-muted)", marginTop: 2 }}>The large banner at the top of every company profile. It shows only when an employer hasn't uploaded their own cover photo &mdash; their own cover always wins. Leave empty to use Krama's built-in branded fallback.</p>
                <div style={{ display: "inline-flex", alignItems: "center", gap: 5, marginTop: 8, fontSize: "var(--text-xs)", fontWeight: 700, color: "var(--text-brand)", background: "var(--brand-subtle)", borderRadius: "var(--radius-pill)", padding: "3px 10px" }}>{I("image", 13)} Recommended image: 1600 × 360px</div>
              </div>
            </div>
            <div style={{ marginTop: 12, paddingTop: 14, borderTop: "1px solid var(--border-subtle)", display: "flex", alignItems: "center", gap: 12 }}>
              <div style={{ width: 96, height: 54, flexShrink: 0, borderRadius: "var(--radius-md)", border: "1px solid var(--border)", background: cphImg ? "transparent" : "var(--surface-sunken)", backgroundImage: cphImg ? "url('" + cphImg + "')" : "none", backgroundSize: (cph.fit === "contain" ? "contain" : "cover"), backgroundPosition: "center", backgroundRepeat: "no-repeat", display: "flex", alignItems: "center", justifyContent: "center" }}>
                {cph._uploading ? <span style={{ fontSize: 10, color: "var(--text-muted)", fontFamily: "var(--font-sans)" }}>Uploading…</span> : (!cphImg && <span style={{ fontSize: 10, color: "var(--text-faint)", fontFamily: "var(--font-sans)" }}>Default</span>)}
              </div>
              <div style={{ flex: 1 }}>
                <label style={{ display: "inline-flex", alignItems: "center", gap: 6, cursor: cph._uploading ? "not-allowed" : "pointer", opacity: cph._uploading ? 0.5 : 1, fontFamily: "var(--font-sans)", fontSize: "var(--text-sm)", fontWeight: 700, color: "var(--text-brand)" }}>
                  {I("image", 14)} {cph._uploading ? "Uploading…" : cphImg ? "Replace default banner" : "Upload default banner"}
                  <input type="file" accept="image/*" disabled={!!cph._uploading} onChange={uploadTo("companyProfileHero")} style={{ display: "none" }} />
                </label>
                {cphImg ? (
                  <div style={{ display: "flex", gap: 6, marginTop: 8 }}>
                    {["cover", "contain"].map((f) => (
                      <button key={f} onClick={() => setBanner("companyProfileHero", "fit", f)} style={{ height: 28, padding: "0 12px", borderRadius: "var(--radius-sm)", cursor: "pointer", border: "1px solid " + ((cph.fit || "cover") === f ? "var(--brand)" : "var(--border-strong)"), background: (cph.fit || "cover") === f ? "var(--brand-subtle)" : "var(--surface-card)", color: (cph.fit || "cover") === f ? "var(--text-brand)" : "var(--text-muted)", fontFamily: "var(--font-sans)", fontSize: "var(--text-xs)", fontWeight: 700 }}>{f}</button>
                    ))}
                    <button onClick={() => setBanner("companyProfileHero", "image", "")} style={{ height: 28, padding: "0 12px", borderRadius: "var(--radius-sm)", cursor: "pointer", border: "1px solid var(--border-strong)", background: "var(--surface-card)", color: "var(--text-muted)", fontFamily: "var(--font-sans)", fontSize: "var(--text-xs)", fontWeight: 700 }}>Remove</button>
                  </div>
                ) : <div style={{ fontSize: "var(--text-xs)", color: "var(--text-muted)", marginTop: 6 }}>Currently showing Krama's built-in branded fallback.</div>}
              </div>
            </div>
          </Card>
          );
  ...
  // Account & data deletion requests (compliance loop). Candidates self-delete; employers and
  // anyone who can't sign in land here as a request an admin completes (erases personal data)
  // or rejects.
  function DeletionRequests() {
    ...
    const load = React.useCallback(function () {
      setLoading(true);
      adm.fetchDeletionRequests(filter).then(function (d) {
    ...
    const complete = function (r) {
      if (!window.confirm("Complete this request?\n\nThis permanently erases " + r.email + "'s personal data (name, email, phone, résumé, alerts) and cannot be undone. Company and billing records are kept.")) return;
      setBusyId(r.id);
      adm.completeDeletionRequest(r.id).then(function () { setBusyId(null); load(); })
        .catch(function (e) { setBusyId(null); alert((e && e.message) || "Failed."); });
    };
    const reject = function (r) {
      if (!window.confirm("Reject this deletion request from " + r.email + "?")) return;
      setBusyId(r.id);
      adm.rejectDeletionRequest(r.id).then(function () { setBusyId(null); load(); })
        .catch(function (e) { setBusyId(null); alert((e && e.message) || "Failed."); });
    };

    const fmtDate = function (s) { if (!s) return "—"; var d = new Date(s); return isNaN(d) ? "—" : d.toLocaleDateString(); };
    const tone = { pending: "warning", done: "success", rejected: "neutral" };

    return (
      <div className="krm-page-pad" style={{ padding: 28, maxWidth: 1000 }}>
        <ScreenHead title="Account deletion requests" sub="Requests from employers (and anyone who can't sign in). Candidates delete themselves instantly." />

        <div style={{ display: "flex", gap: 8, marginBottom: 16 }}>
          {["pending", "all", "done", "rejected"].map(function (f) {
            const on = filter === f;
        ...
        {loading ? (
          ...
          <Card padding={28}><div style={{ color: "var(--text-muted)", textAlign: "center" }}>No {filter === "all" ? "" : filter + " "}requests.</div></Card>
        ) : (
          <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
            {rows.map(function (r) {
              return (
                  ...
                  <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between", gap: 16, flexWrap: "wrap" }}>
                    <div style={{ minWidth: 0 }}>
                      <div style={{ display: "flex", alignItems: "center", gap: 8, flexWrap: "wrap" }}>
                        <span style={{ fontWeight: 700, color: "var(--text-strong)" }}>{r.email}</span>
                        <Badge tone={tone[r.status] || "neutral"} style={{ textTransform: "capitalize" }}>{r.status}</Badge>
                        {r.role && <Badge tone="neutral" style={{ textTransform: "capitalize" }}>{r.role}</Badge>}
                        {r.already_erased && <Badge tone="success">Erased</Badge>}
                      </div>
                      <div style={{ fontSize: "var(--text-sm)", color: "var(--text-muted)", marginTop: 4 }}>
                        Requested {fmtDate(r.created_at)}{r.handled_at ? " · handled " + fmtDate(r.handled_at) : ""}
                        {r.user ? " · account: " + r.user.name + " (" + r.user.status + ")" : " · account not found"}
                      </div>
                      {r.reason && <div style={{ fontSize: "var(--text-sm)", color: "var(--text-body)", marginTop: 6, fontStyle: "italic" }}>“{r.reason}”</div>}
                    </div>
                    {r.status === "pending" && (
                      <div style={{ display: "flex", gap: 8, flexShrink: 0 }}>
                        <Button variant="primary" size="sm" style={{ background: "var(--danger)", borderColor: "var(--danger)" }} disabled={busyId === r.id} onClick={function () { complete(r); }}>{busyId === r.id ? "Working…" : "Complete & erase"}</Button>
